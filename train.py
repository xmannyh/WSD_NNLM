...
def main():
    ...
    parser.add_argument('--wrd_embedding_size', type=int, default=512,
                        help='size of the context layer')
    ...
    parser.add_argument('--stop_count', type=int, default=3,
                       help='how many epochs to wait for the model to improve on the dev set')
    parser.add_argument('--data_set_size', type=int, default=500,
    ...
    parser.add_argument('--learning_rate', type=float, default=0.001,
                       help='learning rate')
    parser.add_argument("--func", type=str, help="Hidden activation (in the set {sigmoid, tanh, relu})",\
                        default="tanh")
...
def train(args):
    ...
    data_loader = TextLoader(args.data_dir, args.sense_file, args.batch_size, args.seq_length, args.data_set_size)
    ...
    with tf.Graph().as_default():
        with tf.Session() as sess:
            ...
            feed_dict_dev = {model.x: x_batch_dev, model.y: y_batch_dev, model.keep_prob: args.keep_prob}
            data_loader.reset_batch_pointer()
            x_batch_test, y_batch_test, unk_count_test, n_sent_test, _ = data_loader.next_batch_test(set_to_choose=1)
            feed_dict_test = {model.x: x_batch_test, model.y: y_batch_test, model.keep_prob: args.keep_prob}
            data_loader.reset_batch_pointer()
            ...
            for epoch in range(args.num_epochs):
                ...
                for i in range(data_loader.num_batches):
                    ...
                    print("training step {}, epoch {}, batch {}/{}, loss: {:.4f}, accuracy: {:.4f}, avg unk count: {}, avg sent: {}, time/batch: {:.3f}"
                        .format(step, epoch, i, data_loader.num_batches, loss, np.mean(equal), int(unk_count/args.batch_size), n_sent, time.time()-start))
                ...
                for i in range(data_loader.num_batches_test):
                    start = time.time()
                    step, summary, loss, equal = sess.run([global_step, merged, model.cost, model.equal], feed_dict_dev)
                   
                    print("dev step {}, epoch {}, batch {}/{}, loss: {:.4f}, accuracy: {:.4f}, avg unk count: {}, avg sent: {}, time/batch: {:.3f}"
                        .format(step, epoch, i, data_loader.num_batches_test, loss, np.mean(equal), int(unk_count_test/args.batch_size), n_sent_test, time.time()-start))
            ...
            for i in range(data_loader.num_batches_test):
                start = time.time()
                step, summary, loss, equal = sess.run([global_step, merged, model.cost, model.equal], feed_dict_test)
               
                print("test step {}, epoch {}, batch {}/{}, loss: {:.4f}, accuracy: {:.4f}, avg unk count: {}, avg sent: {}, time/batch: {:.3f}"
                    .format(step, epoch, i, data_loader.num_batches_test, loss, np.mean(equal), int(unk_count_test/args.batch_size), n_sent_test, time.time()-start))
                accur.append(np.mean(equal))
